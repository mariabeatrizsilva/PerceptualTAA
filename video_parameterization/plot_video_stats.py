...
def plot_coverage(df, features):
    scaler = StandardScaler()
    scaled = scaler.fit_transform(df[features])
    scaled_df = pd.DataFrame(scaled, columns=features, index=df.index)

    fig, ax = plt.subplots(figsize=(10, len(features) * 0.9 + 1))

    colors = plt.cm.GnBu(np.linspace(0.2, 0.9, len(df)))

    for i, feat in enumerate(reversed(features)):
        values = scaled_df[feat].values
        y = np.full(len(values), i)
        # Jitter slightly so overlapping points don't stack
        y_jitter = y + np.random.uniform(-0.15, 0.15, size=len(values))
        ax.scatter(values, y_jitter, color=colors, s=60, alpha=0.85, zorder=3)

    ax.set_yticks(range(len(features)))
    ax.set_yticklabels([PARAM_LABELS[f] for f in reversed(features)], fontsize=10)
    ax.axvline(0, color="#333", linewidth=0.8, linestyle="--")
    ax.set_xlabel("z-score", fontsize=10)
    ax.set_title("Dataset coverage across video parameters", fontsize=12, fontweight="bold")
    ax.grid(axis="x", linestyle="--", alpha=0.4)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
...
def main(csv_path):
    ...
    save(plot_coverage(df, features),      "coveragemap.png")
